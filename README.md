...
- [x] add feature to send lines cleared to opponent - see: https://tetris.wiki/Garbage
  ...
  - [ ] Fix bug in opponent rendered game where the piece placed + garbage received events aren't being processed in the correct order.
    - this is happening because the onSendGarbage function is called before the other actions (like piece locking) are captured and sent. Eg:
    ```ts
    ...
    ```
    or not, dang.
    Essentially the issue is a visual one, just with the last piece that is placed before the garbage is added. that's it.
